...
def get_FLAGS():
    ...
    tf.app.flags.DEFINE_integer('seed', 2024, """Seed. """)
    ...
    tf.app.flags.DEFINE_integer('output_delay', 5, """Number of iterations between log/loss outputs. """)
    ...
    tf.app.flags.DEFINE_float('lrate', 2e-4, """Learning rate. """)
    ...
    tf.app.flags.DEFINE_string('outdir', '../Data/DRNet/results/', """Output directory. """)
    tf.app.flags.DEFINE_string('datadir', '../Data/DRNet/data/Syn_1.0_1.0_0/2_4_4/', """Data directory. """)
    ...
    tf.app.flags.DEFINE_boolean('reweight_sample', 0, """Whether to reweight sample for prediction loss with average treatment probability. """)
    tf.app.flags.DEFINE_boolean('twoStage', 1, """twoStage. """)
    tf.app.flags.DEFINE_boolean('latent', 1, """Latent Model. """)
    tf.app.flags.DEFINE_string('f', '', 'kernel')
    tf.app.flags.DEFINE_string('mode', '', 'kernel')
    tf.app.flags.DEFINE_string('num_reps', '', 'kernel')
...
class GCBIV(object):
    ...
    def _build_graph(self, x, s, t, y_ , p_t, FLAGS, r_alpha, r_lambda, do_in, do_out, dims):
        ...
        if FLAGS.twoStage:
            if FLAGS.latent:
                ws = s * t + (1-s) * (1-t)
                sample_weight = sample_weight * ws
            else:
                y = s * y1 + (1-s) * y0
...
def trainNet(Net, sess, train_step, train_data, val_data, test_data, FLAGS, logfile, _logfile, exp):
    ...
    # In evaluation mode, we need to use the factual outcome to early stop the results, rather than the estimated s=hat_t
    ...
    for i in range(FLAGS.iterations):
        ...
        if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
            ...
            # wws = sess.run(Net.ws, feed_dict={Net.x: train_data['x'], Net.s: train_data['s'], Net.t: train_data['t'], Net.do_in: 1.0, Net.do_out: 1.0})
            # print(wws[:10])

            final[i] = {'best':valid_f_error, 'ate_train': np.mean(y_pred_mu1) - np.mean(y_pred_mu0), 'ate_test': np.mean(y_pred_mu1_test) - np.mean(y_pred_mu0_test), 'itr': i,
            ...
            loss_str = str(i) + '\tObj: %.3f,\tF: %.3f,\tCf: %.3f,\tImb: %.2g,\tVal: %.3f,\tValImb: %.2g,\tValObj: %.2f,\tate_train: %.2g,\tate_test: %.2f' \
                    % (obj_loss, f_error, cf_error, imb_err, valid_f_error, valid_imb, valid_obj, final[i]['ate_train'], final[i]['ate_test'])
...
def run(exp, args, dataDir, resultDir, train, val, test, device, input_ihdp=False):
    ...
    _logfile = f'{resultDir}/GCBIV.txt'
    ...
    Net = GCBIV(x, s, t, y_, p, FLAGS, r_alpha, r_lambda, do_in, do_out, dims)
