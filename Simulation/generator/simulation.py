...
from .utils import plot,lindisc_np,ACE,get_var_df

class Simulation(object):
    def __init__(self, n=10000, add=0, sc=1, sh=0, one=1, depX=0.05, depU=0.05,
                 VX=1, mV=2, mX=4, mU=4, init_seed=7, seed_coef=10,storage_path='./Data/'):
        self.n = n
        self.add = add
        self.sc = sc
        self.sh = sh
    ...
    def set_path(self, showPath=True):
    ...
    def get_data(self, n, mu, sig, seed):
        ...
        mu_1 = np.dot(Y_vars**2, self.coefs_XU1) / (mX+mU) + self.add
...
if __name__ == "__main__":
    ToyData = Simulation(n=10000, add=0,sc=1,sh=0,one=1,depX=0.05,depU=0.05,VX=1,mV=2,mX=4,mU=4,init_seed=7,seed_coef=10,storage_path='./Data/')
    ToyData.run(n=10000, num_reps=10)